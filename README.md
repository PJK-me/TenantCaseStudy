...
You can access them like this:  
    http://localhost/api/  
    http://tenant1.localhost/api/    http://tenant2.localhost/api/  
...
Admin users can create new tenants using /api/tenants/ by providing data:  
    {  
       "name": <new_tenant_name>,  
       "domain_url" <url_of_for_new_domain>  
    }  
  
Example:  
    Assuming admin created new tenant with new domain url = "new_domain" - Admin will be able to access and refresh tokens using curl:  
    curl -X POST http://localhost/api/token/ \  
    -H "Content-Type: application/json" \  
    -H "Host: new_domain" \  
    -d '{"username": "tenant_user_1","password":"password"}'  
